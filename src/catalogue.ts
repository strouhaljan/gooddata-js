...
export class CatalogueModule {
    ...
    public loadItems(projectId: string, options: ILoadCatalogOptions = {}) {
        ...
        if (hasBuckets) {
            return this.bucketItemsToExecConfig(projectId, mdObj, { attributesMap })
                .then((bucketItems: any) => this.loadCatalog(projectId, { ...request, bucketItems }));
        }
    ...
    public loadDateDataSets(projectId: string, options: ILoadDateDataSetOptions) {
        ...
        const bucketItemsPromise = mdObj
            ? this.bucketItemsToExecConfig(
                projectId,
                mdObj,
                { removeDateItems: true, attributesMap: get(options, 'attributesMap') }
            )
            : Promise.resolve();
    ...
    private requestDateDataSets(projectId: string, dateDataSetsRequest: any) {
        ...
        return this.xhr.post(uri, { data: { dateDataSetsRequest } })
            .then((r => r.getData()))
    ...
    private loadCatalog(projectId: string, catalogRequest: any) {
        ...
        return this.xhr.post(uri, { data: { catalogRequest } })
            .then((r => r.getData()))
